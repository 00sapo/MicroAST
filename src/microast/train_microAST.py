...
# training options
parser.add_argument("--save_dir", default="./exp", help="Directory to save the models")
...
parser.add_argument(
    "--resume", action="store_true", help="train the model from the checkpoint"
)
parser.add_argument(
    "--checkpoints", default="./checkpoints", help="Directory to save the checkpoint"
)
...
for i in tqdm(range(start_iter + 1, args.max_iter)):
    ...
    stylized_results, loss_c, loss_s, loss_contrastive = network(
    ...
    if (i + 1) % 500 == 0:
        ...
        print(
            "[%d/%d] loss_content:%.4f, loss_style:%.4f, loss_contrastive:%.4f"
            % (
                i + 1,
                args.max_iter,
                loss_c.item(),
    ...
    if (i + 1) % args.save_model_interval == 0 or (i + 1) == args.max_iter:
        ...
        torch.save(
            state_dict, save_dir / "content_encoder_iter_{:d}.pth.tar".format(i + 1)
        )
        ...
        for key in state_dict.keys():
            state_dict[key] = state_dict[key].to(torch.device("cpu"))
        torch.save(
            state_dict, save_dir / "style_encoder_iter_{:d}.pth.tar".format(i + 1)
        )

        state_dict = network.modulator.state_dict()
        for key in state_dict.keys():
            state_dict[key] = state_dict[key].to(torch.device("cpu"))
        torch.save(state_dict, save_dir / "modulator_iter_{:d}.pth.tar".format(i + 1))
